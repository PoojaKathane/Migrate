...
$localPath={Provide local path where packages are downloaded}
       ...
       $projectResult1.value | ForEach-Object{
        ...
        if($sourceProjectName -eq "$targetProject"){
            ForEach($project in $targetProject){
                 ...
                 $destFeedName="$Feed1" 
                 $feedName = $destFeedName+$targetProject
                 $body = @{
                    ...
                    $destfeedurl ="https://feeds.dev.azure.com/$destOrgName/$targetProject/_apis/packaging/feeds/$feedName ?api-version=7.0"
